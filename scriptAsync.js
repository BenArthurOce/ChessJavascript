...
import Parser from "./libs/StaticChessParser.js";
...
import ChessUtility from "./libs/StaticChessUtility.js";

import OpeningDatabase from "./libs/Openings.js";

import HTMLFrontPage from "./libs/_HTMLFrontPage.js";
...
const myOpenings = [a, b, c, f, h, i, j, k, m, n]
...
myOpenings.forEach(async (opening) => {
    ...
    try {
        // Logic() Contains the Board() and the Parser()
        const newLogic = new Logic(opening);

        // in processMovesAsync, Logic().processAllMoves is ran Asynchronously
        await processMovesAsync(newLogic);

        // A Chessboard is created
        const chessHTMLBoard = new HTMLBoard(100, 100);
        document.body.querySelector("#side-board-containers").appendChild(chessHTMLBoard.element);

        // Asynchronously render the chessboard
        await renderAsync(chessHTMLBoard, newLogic.gameBoard.grid);
    } catch (error) {
...
additionalChessHTML.renderToDOM(additionalLogic.gameBoard.grid);


// additionalLogic.gameBoard.printToTerminal()
// console.log(additionalLogic.gameBoard.grid)
...
//
  ...
  button.addEventListener('click', (event) => {
    console.log("hello")
    // console.log(openingDB.openings)
    console.log(openingDB.filterOpeningDict("ECO", "A00", "4"))
  });


const testFrontPage = new HTMLFrontPage()